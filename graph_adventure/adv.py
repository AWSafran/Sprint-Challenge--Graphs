...
def explore_room(previous_room, graph, player, path, direction = None):
    ...
    #Need to assign room relationships
    if direction is not None: #Meaning we've moved, rather than our first call to recursion
        graph[previous_room][direction] = current_room
        graph[current_room][rev_dirs[direction]] = previous_room



    if '?' not in list(graph[current_room].values()):
        # We don't need to do anything but return here, we move back after this recursive call ends anyway
        # path.append(rev_dirs[direction])
        # player.travel(rev_dirs[direction])
        return

    else:
        random_keys = list(graph[current_room])
        random.shuffle(random_keys)
        for key in random_keys:
            if graph[current_room][key] == '?':
                path.append(key)
                player.travel(key)
...
explore_room(player.currentRoom.id, traversal_graph, player, traversalPath)
